...
namespace Components
{
	...
	void Console::RefreshStatus()
	{
		...
		else if (IsWindow(GetWindow()) != FALSE)
		{
			...
			SetWindowTextA(GetWindow(), Utils::String::Format("IW4C6 " REVISION_STR "-develop : {}", hostname));
#else
			SetWindowTextA(GetWindow(), Utils::String::Format("IW4C6 " REVISION_STR " : {}", hostname));
#endif
		}
	}
	...
	Console::Console()
	{
		...
		// Console '%s: %s> ' string
#ifdef EXPERIMENTAL_BUILD
		Utils::Hook::Set<const char*>(0x5A44B4, "IW4C6: " REVISION_STR "-develop> ");
#else
		Utils::Hook::Set<const char*>(0x5A44B4, "IW4C6: " REVISION_STR "> ");
#endif
